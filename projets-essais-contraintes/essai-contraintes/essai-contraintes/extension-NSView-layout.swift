...
extension NSView {
  ...
  func displayConstraints (_ inDisplayDictionary : [ObjectIdentifier : String]) {
    ...
    if !self.constraintsAffectingLayout (for: .horizontal).isEmpty {
      Swift.print ("  Horizontal:")
      for c in self.constraintsAffectingLayout (for: .horizontal) {
        displayConstraint (c, inDisplayDictionary)
      }
    }
    if !self.constraintsAffectingLayout (for: .vertical).isEmpty {
      Swift.print ("  Vertical:")
      for c in self.constraintsAffectingLayout (for: .vertical) {
        displayConstraint (c, inDisplayDictionary)
      }
    }
  }
